...
if "%NEED_INSTALL%"=="1" (
    echo [1/2] Installing/updating dependencies, please wait 1-3 min...
    call npm install
    if errorlevel 1 (
        echo.
...
REM -- build --
echo [2/2] Building Vue admin UI...
call npm run build
if errorlevel 1 (
...
echo.
echo ====================================================
echo [DONE] Admin UI built to platform\admin-vue\dist\
echo        Next start_platform.bat will serve it at:
echo        http://127.0.0.1:8000/admin-ui/
echo ====================================================
echo.
pause
